...
import questionBank from "../../data/web-development.json";
...
export default function WebDevelopmentTest() {
  ...
  useEffect(() => {
    ...
    const blockKeys = (e) => {
      ...
      if (forbidden) {
        triggerAutoSubmit(
          "You violated the exam rules, so the Web Development test was automatically submitted. Kindly contact HR."
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimer((prev) => {
        if (prev <= 1) {
          triggerAutoSubmit(
            "The time is over, so the Web Development test was automatically submitted. Please contact HR for further details."
          );
    ...
    const handleVisibility = () => {
      if (document.visibilityState === "hidden") {
        setTabCount((prev) => {
          ...
          triggerAutoSubmit(
            "You violated the rules 3 times, so the Web Development test was submitted. Kindly contact HR."
          );
          return count;
        });
      }
  ...
  if (step === "submitted") {
    return (
      <CenteredCard title="Web Development Test Submitted">
        <p className="text-center text-red-600 font-semibold mb-6">
          {autoSubmitReason ||
            "Your Web Development test was successfully submitted."}
        </p>
  ...
  if (step === "instructions") {
    return (
      <CenteredCard title="Web Development Test Instructions">
        ...
        <PrimaryButton
          className="mt-6"
          onClick={() =>
            window.open("/test/web-development?start=true", "_blank")
          }
        >
          Start Web Development Test
        </PrimaryButton>
      </CenteredCard>
    );
  }
  ...
  if (step === "login") {
    return (
      <CenteredCard title="Web Development Online Assessment">
        <form
          onSubmit={(e) => {
            e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-3 py-4 select-none">
      <div className="sticky top-0 bg-white shadow rounded-xl px-4 py-3 flex justify-between">
        <h2 className="font-bold">Web Development Test</h2>
