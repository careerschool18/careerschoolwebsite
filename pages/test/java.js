...
import questionBank from "../../data/java-questions.json";
...
export default function JavaTest() {
  ...
  useEffect(() => {
    ...
    const blockKeys = (e) => {
      ...
      if (forbidden) {
        triggerAutoSubmit(
          "You violated the exam rules, so the Java test was automatically submitted. Kindly contact HR."
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimer((prev) => {
        if (prev <= 1) {
          triggerAutoSubmit(
            "The time is over, so the Java test was automatically submitted. Please contact HR for further details."
          );
    ...
    const handleVisibility = () => {
      if (document.visibilityState === "hidden") {
        setTabCount((prev) => {
          ...
          triggerAutoSubmit(
            "You violated the rules 3 times, so the Java test was submitted. Kindly contact HR."
          );
          return count;
        });
      }
  ...
  if (step === "submitted") {
    return (
      <CenteredCard title="Java Test Submitted">
        <p className="text-center text-red-600 font-semibold mb-6">
          {autoSubmitReason || "Your Java test was successfully submitted."}
        </p>
  ...
  if (step === "instructions") {
    return (
      <CenteredCard title="Java Test Instructions">
        ...
        <PrimaryButton
          className="mt-6"
          onClick={() =>
            window.open("/test/java?start=true", "_blank")
          }
        >
          Start Java Test
        </PrimaryButton>
  ...
  if (step === "login") {
    return (
      <CenteredCard title="Java Online Assessment">
        <form
          onSubmit={(e) => {
            e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-3 py-4 select-none">
      <div className="sticky top-0 bg-white shadow rounded-xl px-4 py-3 flex justify-between">
        <h2 className="font-bold">Java Test</h2>
