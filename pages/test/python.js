...
import questionBank from "../../data/python-questions.json";
...
export default function PythonTest() {
  ...
  useEffect(() => {
    ...
    const blockKeys = (e) => {
      ...
      if (forbidden) {
        triggerAutoSubmit(
          "You violated the exam rules, so the Python test was automatically submitted. Kindly contact HR."
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimer((prev) => {
        if (prev <= 1) {
          triggerAutoSubmit(
            "The time is over, so the Python test was automatically submitted. Please contact HR for further details."
          );
    ...
    const handleVisibility = () => {
      if (document.visibilityState === "hidden") {
        setTabCount((prev) => {
          ...
          triggerAutoSubmit(
            "You violated the rules 3 times, so the Python test was submitted. Kindly contact HR."
          );
          return count;
        });
      }
  ...
  if (step === "submitted") {
    return (
      <CenteredCard title="Python Test Submitted">
        <p className="text-center text-red-600 font-semibold mb-6">
          {autoSubmitReason ||
            "Your Python test was successfully submitted."}
        </p>
  ...
  if (step === "instructions") {
    return (
      <CenteredCard title="Python Test Instructions">
        ...
        <PrimaryButton
          className="mt-6"
          onClick={() =>
            window.open("/test/python?start=true", "_blank")
          }
        >
          Start Python Test
        </PrimaryButton>
  ...
  if (step === "login") {
    return (
      <CenteredCard title="Python Online Assessment">
        <form
          onSubmit={(e) => {
            e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-3 py-4 select-none">
      <div className="sticky top-0 bg-white shadow rounded-xl px-4 py-3 flex justify-between">
        <h2 className="font-bold">Python Test</h2>
