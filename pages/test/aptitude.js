...
import questionBank from "../../data/aptitude-questions.json";
...
export default function AptitudeTest() {
  ...
  useEffect(() => {
    ...
    const blockKeys = (e) => {
      ...
      if (forbidden) {
        triggerAutoSubmit(
          "You violated the exam rules, so the Aptitude test was automatically submitted. Kindly contact HR."
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimer((prev) => {
        if (prev <= 1) {
          triggerAutoSubmit(
            "The time is over, so the Aptitude test was automatically submitted. Please contact HR for further details."
          );
    ...
    const handleVisibility = () => {
      if (document.visibilityState === "hidden") {
        setTabCount((prev) => {
          ...
          triggerAutoSubmit(
            "You violated the rules 3 times, so the Aptitude test was submitted. Kindly contact HR."
          );
          return count;
        });
      }
  ...
  if (step === "submitted") {
    return (
      <CenteredCard title="Aptitude Test Submitted">
        <p className="text-center text-red-600 font-semibold mb-6">
          {autoSubmitReason ||
            "Your Aptitude test was successfully submitted."}
        </p>
  ...
  if (step === "instructions") {
    return (
      <CenteredCard title="Aptitude Test Instructions">
        ...
        <PrimaryButton
          className="mt-6"
          onClick={() =>
            window.open("/test/aptitude?start=true", "_blank")
          }
        >
          Start Aptitude Test
        </PrimaryButton>
  ...
  if (step === "login") {
    return (
      <CenteredCard title="Aptitude Online Assessment">
        <form
          onSubmit={(e) => {
            e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-3 py-4 select-none">
      <div className="sticky top-0 bg-white shadow rounded-xl px-4 py-3 flex justify-between">
        <h2 className="font-bold">Aptitude Test</h2>
